...
各自の当月の入金額の比率を「負担比率」とし、当月の共通支出（立替）の総額をその比率で分け合い、実際の立替額との差額から最終的な送金額と送金方向を決定します。
...
* **累積バーチャル共通口座の計算:**
  選択された月以前（選択された月を含む）のデータから算出される累積のバーチャル共通口座の情報です。
  * **累積プール金総額 ($Pool_{inc\_cumulative}$)**: 選択月以前の「入金（収入）」の合計額。
  * **累積支出総額 ($Pool_{exp\_cumulative}$)**: 選択月以前の「共通支出（立替）」の合計額。
  * **現在の口座残高（累積残高） ($Pool_{remaining\_cumulative}$)**:
    $$Pool_{remaining\_cumulative} = Pool_{inc\_cumulative} - Pool_{exp\_cumulative}$$
...
* **Virtual Shared Account Card:**
  ...
  * 表示項目：累積プール金（全期間の入金合計）、今月の支出合計（選択された月のみの支出合計）、現在の口座残高（累積プール金 - 全期間の支出）。
